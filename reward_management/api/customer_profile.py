...
# get customer user detail for admin side customer details
@frappe.whitelist()
def show_customer_user(carpenterName):
    try : 
        # Fetch specific fields from User document based on email
...
# update customer profile through admin---------------
@frappe.whitelist()
        ...
        if old_email != new_email:
            ...
            if user.mobile_no:
                ...
                if customer:
                    ...
                    customer_doc.first_name = user.first_name
                    customer_doc.last_name = user.last_name
                    customer_doc.full_name = user.full_name
                    customer_doc.city = user.location
                    customer_doc.email = new_email
                    customer_doc.gender = user.gender
                    customer_doc.birth_date = user.birth_date

                    if user_data.get('user_image'):
                        customer_doc.image = user_data.get('user_image')
                     # Clear FCM token on logout
                    customer_doc.fcm_token = ""
                    # clear web fcm token on logout
                    customer_doc.web_fcm_token = ""
...
@frappe.whitelist()
def update_customer_image(name, new_image_url):
    try:
        # Fetch User document based on name
        ...
        if user.mobile_no:
            ...
            if customer:
                ...
                # Update Customer's image field (assuming 'image' is the field name for the image in Customer)
                customer_doc.image = new_image_url
                # Save Customer document changes
                customer_doc.save()

        return {"status": "success", "message": "Customer image updated successfully."}
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), _("API Error"))
        return {"status": "error", "message": str(e)}
...
@frappe.whitelist()
def remove_customer_image(name):
    try:    
        # Fetch User document based on name
        user = frappe.get_doc("User", {"mobile_no": name})
        ...
        if user.mobile_no:
            ...
            if customer:
                ...
                # Reset or remove Customer's image field (assuming 'image' is the field name for image in Customer)
                customer_doc.image = None
                # Save Carpenter document changes
                customer_doc.save()

        return {"status": "success", "message": "Customer image removed successfully."}
