...
def create_new_carpainters(firstname, lastname, city, mobile):
    ...
    if frappe.db.exists("Customer", {"mobile_number": mobile}):
            return {"success": False, "status": "failed", "message": "Customer already exists. Please login."}
    ...
    lock_acquired = frappe.cache().set(lock_key, 1, nx=True, ex=5)
    if not lock_acquired:
        frappe.log_error(f"Duplicate registration attempt detected for mobile: {mobile}")
        # frappe.logger().warning(f"Duplicate request detected for mobile: {mobile}")
        return {
            "success": False,
            "status": "failed",
    ...
    try:
        ...
        frappe.log_error(f"Error Creating New Customer: {str(e)}")
        return {"success": False, "status": "failed", "message": str(e)}
